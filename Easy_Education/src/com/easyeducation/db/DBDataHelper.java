...
public final class DBDataHelper {
	 ...
	 * @return
	 */
	public ArrayList<Module> select(String tableName, String showColumns,
	 ...
	 */
	public ArrayList<Module> select(final String table, final String selection,
	...
	private void changeToList(Cursor cursor, List<Module> modules,
			Class<?> moduleClass) {
		// 取出所有的列名
	...
	private Module changeToModule(Cursor cursor, Class<?> moduleClass)
	...
	public long update(final String table, final Module module) {
		synchronized (dbHelper) {
			ContentValues values = moduleToContentValues(module);
			return dbHelper.update(table, values, DBHelper.ID + "=?",
					new String[] { module.id });
		}
	}
	...
	public int delete(final String table, final Module module) {
		synchronized (dbHelper) {
			return dbHelper.delete(table, "id=?", new String[] { module.id });
		}
	}
